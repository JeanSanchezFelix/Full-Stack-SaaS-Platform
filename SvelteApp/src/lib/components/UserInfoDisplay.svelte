  ...
  {#if customer}
    <div class="flex gap-4 flex-row items-start justify-between">
      <div class="flex gap-4">
        <div
          class="flex h-12 w-12 shrink-0 items-center justify-center rounded-md bg-slate-950 text-sm font-bold text-white"
        >
          {initials}
        </div>
        <div>
          <!-- <p class="text-xs font-semibold uppercase text-slate-500">Cuenta encontrada</p> -->
          <h2 class="text-lg font-bold tracking-normal text-slate-950">
            {fullName}
          </h2>
          <p class="mt-1 text-xs text-slate-600">{customer.customerCode}</p>
        </div>
      </div>
    ...
    <dl
      class="mt-5 grid gap-4 border-t border-slate-200 pt-4 text-sm md:grid-cols-2 lg:grid-cols-3"
    >
      <div>
        <dt class="font-semibold text-slate-800">Licencia</dt>
        <dd class="mt-1 text-slate-600">
          {customer.licenseNumber || "No registrada"}
        </dd>
      </div>
      <div>
        <dt class="font-semibold text-slate-800">Teléfono</dt>
        <dd class="mt-1 text-slate-600">
          {customer.phoneNumber || "No registrado"}
        </dd>
      </div>
      <div>
        <dt class="font-semibold text-slate-800">Correo electrónico</dt>
        <dd class="mt-1 break-words text-slate-600">
...
{#if showBookings}
  ...
  >
    <div
      class="w-full max-w-3xl rounded-md border border-slate-200 bg-white p-4 shadow-sm"
    >
              ...
              {#each customer.bookings as booking}
                <tr>
                  <td class="px-3 py-2 text-slate-700"
                    >{new Date(booking.requestedStart).toLocaleDateString(
                      "es-PR",
                    )}</td
                  >
                  <td class="px-3 py-2 text-slate-700"
                    >{new Date(booking.requestedStart).toLocaleTimeString(
                      "es-PR",
                  ...
                  <td class="px-3 py-2 text-slate-700"
                    >{booking.requestedEnd
                      ? new Date(booking.requestedEnd).toLocaleTimeString(
                          "es-PR",
                          { hour: "2-digit", minute: "2-digit" },
                        )
                      : "-"}</td
                  ...
                  <td class="px-3 py-2 text-slate-700"
                    >{booking.requestedEnd
                      ? Math.round(
                          (new Date(booking.requestedEnd) -
                            new Date(booking.requestedStart)) /
                            3600000,
                        )
                      : "-"} h</td
                  >
                  <td class="px-3 py-2 text-slate-700"
                    >{booking.scooterQuantity}</td
                  >
                  <td class="px-3 py-2 text-slate-700"
                    >{booking.ebikeQuantity}</td
                  >
                  <td class="px-3 py-2 font-medium text-slate-800"
                    >{booking.status}</td
                  >
                  <td class="px-3 py-2 text-slate-600"
                    >{booking.adminNotes || "-"}</td
                  >
                  <td class="px-3 py-2 text-right">
                    {#if booking.canDelete}
                      <a
                        class="rounded-md bg-slate-100 px-2 py-1 text-xs font-semibold text-slate-700 hover:bg-slate-200"
                        href={`/Bookings/DeleteOwn?id=${booking.id}&licenseNumber=${encodeURIComponent(customer.licenseNumber || "")}`}
                      >
                        Eliminar
                      </a>
                    {/if}
                    {#if booking.reconfirmRequested}
                      <div class="mt-1 flex gap-1">
                        <a
                          ...
                          href={`/Bookings/RespondReconfirm?id=${booking.id}&accept=true&licenseNumber=${encodeURIComponent(customer.licenseNumber || "")}`}
                        >
                          Aceptar
                        </a>
                        <a
                          class="rounded-md bg-red-100 px-2 py-1 text-xs font-semibold text-red-700 hover:bg-red-200"
                          href={`/Bookings/RespondReconfirm?id=${booking.id}&accept=false&licenseNumber=${encodeURIComponent(customer.licenseNumber || "")}`}
                        >
                          Rechazar
                        </a>
                      </div>
